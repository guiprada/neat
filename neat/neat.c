...
neat_window* neat_window_create(
	...
	if (w == NULL) {
		printf("ERROR: malloc failed in neat_window_create!");
		return NULL;
	}
	...
	if (w->window == NULL) {
		printf("ERROR: creating SDL2 Window failed in neat_window_create!\n");
		return NULL;
	}
	...
	if (w->renderer == NULL) {
		printf("ERROR: creating SDL2 Renderer failed in neat_window_create!\n");
...
void neat_sprite_init(neat_sprite* sprite, neat_texture* tex) {
	...
	sprite->source_rect = neat_SDL_rect_create(0, 0, w, h);
	sprite->pos_rect = neat_SDL_rect_create(0, 0, w, h);
	sprite->pos = neat_vec2_create(0, 0);
	sprite->anchor = neat_vec2_create(w / 2, h / 2);
	sprite->anchor_set = false;
	sprite->rotation = 0;
	sprite->scale = neat_vec2_create(1, 1);
...
SDL_Rect neat_SDL_rect_create(int x, int y, int w, int h) {
	SDL_Rect r;
	r.x = x;
	r.y = y;
	r.w = w;
...
SDL_Point neat_SDL_Point_create(int x, int y) {
	SDL_Point p;
	p.x = x;
	p.y = y;
...
SDL_Point neat_SDL_Point_create_from_neat_vec2(neat_vec2* v) {
	SDL_Point p;
	p.x = (int)v->x;
	p.y = (int)v->y;
...
neat_vec2 neat_vec2_create(double x, double y) {
	neat_vec2 p;
	p.x = x;
	p.y = y;
...
neat_texture* neat_texture_create(neat_window* window, const char* file) {
	...
	if (loaded_surface == NULL) {
		...
		if (new_texture == NULL) {
			printf("ERROR: malloc failed in neat_texture_create!");
			return NULL;
		}
		...
		if (new_texture->texture == NULL) {
			printf("ERROR: creating texture failed!\n");
			free(new_texture);
			return NULL;
		}
...
neat_texture* neat_texture_create_from_text(
	...
	if (text_surface == NULL) {
		...
		if (text_texture == NULL) {
			printf("ERROR: malloc failed in neat_texture_create_from_text");
			return NULL;
		}
...
void  neat_SDL_Rect_fill(neat_window* w, SDL_Color* color, SDL_Rect* rect) {
	SDL_Color  old_color;
